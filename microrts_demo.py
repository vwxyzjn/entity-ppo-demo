...
class GymMicrorts(Environment):
    ...
    def act(self, actions: Mapping[str, Action]) -> Observation:
        ...
        if "unit_action" in actions and isinstance(actions["unit_action"], CategoricalAction):
            unit_action_actors = actions["unit_action"].actors
            unit_actions = actions["unit_action"].indices
        if "base_action" in actions and isinstance(actions["base_action"], CategoricalAction):
            base_action_actors = actions["base_action"].actors
            base_actions = actions["base_action"].indices
        if "barrack_action" in actions and isinstance(actions["barrack_action"], CategoricalAction):
    ...
    def generate_entities(self, response: Any) -> Dict[str, Tuple[Features, Sequence[Sequence[int]]]]:
